...
export function Header() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/coupons/active`).then(r => r.json()).then(j => {
        ...
        <div className="container mx-auto p-4">
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="flex items-center gap-2 group">
             <div>
              <img src={logo} alt="logo" className="w-[160px] md:w-[220px] lg:w-[280px]" />
             </div>
            </Link>
            ...
            <div className="flex items-center gap-2">
              ...
              {/* User / Auth — hide for admin users */}
              {mounted && isLoggedIn() && user?.role !== 'ADMIN' && user?.role !== 'SUPER_ADMIN' ? (
                  ...
                  <AnimatePresence>
                    {showUserMenu && (
                      ...
                      >
                        ...
                        <Link to="/account" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
                          <User className="w-4 h-4 text-indigo-400" /> My Profile
                        </Link>
                        <Link to="/account?tab=orders" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
                          <ShoppingBag className="w-4 h-4 text-indigo-400" /> My Orders
                        </Link>
                        <Link to="/account?tab=password" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
              ...
              <nav className="container mx-auto px-4 py-4 flex flex-col gap-1">
                {navLinks.map((link, i) => (
                  <motion.div key={link.href} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                    <Link to={link.href} className="block py-3 px-4 text-gray-700 font-medium hover:text-navy-700 hover:bg-gray-50 rounded-lg transition-colors" onClick={() => setMobileMenuOpen(false)}>
                      {link.label}
                    </Link>
                  </motion.div>
                ))}
                <div className="pt-3 border-t mt-2 space-y-2">
                  {mounted && isLoggedIn() && user?.role !== 'ADMIN' && user?.role !== 'SUPER_ADMIN' ? (
                    ...
                    <button onClick={() => { openAuthModal(); setMobileMenuOpen(false); }} className="w-full flex items-center justify-center gap-2 py-2.5 px-4 border border-indigo-200 text-indigo-600 rounded-lg text-sm font-medium hover:bg-indigo-50 transition">
