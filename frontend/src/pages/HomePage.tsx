...
const testimonials = [
  { name: 'Priya Sharma', label: 'Verified Buyer • 2 months ago', rating: 5, text: '"The Ortho mattress is a life-changer. My chronic back pain disappeared within a week. Best investment for my health!"', initials: 'PS', color: 'from-[#092f75] to-[#092f75]' },
  { name: 'Rahul Mehta', label: 'Verified Buyer • 2 months ago', rating: 5, text: '"Ordering was seamless and delivery happened on time. The factory-direct price is unbeatable for this quality."', initials: 'RM', color: 'from-[#092f75] to-[#092f75]' },
  { name: 'Ananya Gupta', label: 'Verified Buyer • 2 months ago', rating: 5, text: '"Love the edge support. It feels very premium and hotel-like. The 100-night trial gave me the confidence to buy."', initials: 'AG', color: 'from-[#092f75] to-[#092f75]' },
  { name: 'Karthik Reddy', label: 'Verified Buyer • 3 months ago', rating: 5, text: '"Excellent quality and super fast delivery. The mattress feels exactly like a luxury hotel bed. Very happy!"', initials: 'KR', color: 'from-[#092f75] to-[#092f75]' },
  { name: 'Meena Iyer', label: 'Verified Buyer • 1 month ago', rating: 5, text: '"The memory foam contours perfectly to my body. I haven\'t slept this well in years. Worth every rupee!"', initials: 'MI', color: 'from-[#092f75] to-[#092f75]' },
];

const faqs = [
  { q: 'Which firmness level should I choose?', a: 'If you sleep on your back or stomach, a firm mattress (7-8/10) provides better spinal alignment. Side sleepers benefit from a medium-soft option (4-5/10). Medium (6/10) is a safe all-rounder.' },
  { q: 'What is the benefit of a Hybrid Mattress?', a: 'Hybrid mattresses combine the bounce and support of springs with the pressure relief of foam or latex. You get the best of both worlds — motion isolation, cooling airflow, and contouring comfort.' },
  { q: 'Is a 100-night trial really risk-free?', a: 'Yes. If you are not completely satisfied within 100 nights, we will arrange a free pickup and issue a full refund — no questions asked.' },
];
  ...
  return (
    <div className="relative rounded-3xl overflow-hidden aspect-[5/4]">
      <AnimatePresence mode="wait">
        ...
        >
          <img src={slide.fallback} alt={slide.badge1} className="w-full h-full object-cover" />
        </motion.div>
      </AnimatePresence>

      {slide.star && (
        <div className="absolute top-5 left-5 bg-yellow-400 text-yellow-900 rounded-full w-10 h-10 flex items-center justify-center text-lg shadow-md">★</div>
      )}
      <div className="absolute bottom-12 left-5">
        <span className="bg-white/90 backdrop-blur-sm text-gray-800 text-sm font-semibold px-4 py-2 rounded-full shadow-md">{slide.badge2}</span>
      </div>
      <div className="absolute bottom-12 right-5">
        <span className="bg-[#092f75] text-white text-sm font-semibold px-4 py-2 rounded-full shadow-md">{slide.badge1}</span>
      </div>
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
        {SLIDE_BADGES.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)} className={`rounded-full transition-all duration-300 ${i === current ? 'bg-white w-5 h-2.5' : 'bg-white/50 w-2.5 h-2.5'}`} />
        ))}
      </div>
    </div>
  );
...
function TestimonialSlider() {
  ...
  useEffect(() => {
    const timer = setInterval(() => setCurrent((c) => (c >= maxIndex ? 0 : c + 1)), 4500);
    return () => clearInterval(timer);
  }, [maxIndex]);
  ...
  return (
    <div className="relative">
      <div className="overflow-hidden">
        ...
        >
          {testimonials.map((t, i) => (
            <div key={i} className="flex-shrink-0" style={{ width: `calc((100% - ${gap * (slidesPerView - 1)}px) / ${slidesPerView})` }}>
              <div className="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm h-full flex flex-col">
                <div className="flex items-center gap-4 mb-5">
                  <div className={`w-14 h-14 rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold text-base bg-gradient-to-br ${t.color}`}>{t.initials}</div>
                  <div>
      ...
      <div className="flex items-center justify-center gap-4 mt-8">
        <button onClick={prev} disabled={current === 0} className="w-11 h-11 rounded-full border border-gray-200 flex items-center justify-center hover:border-[#1a2a6c] hover:text-[#1a2a6c] disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
          <ChevronLeft className="w-5 h-5" />
        </button>
        <div className="flex gap-2">
          {Array.from({ length: maxIndex + 1 }).map((_, i) => (
            <button key={i} onClick={() => setCurrent(i)} className={`h-2.5 rounded-full transition-all duration-300 ${i === current ? 'w-7 bg-[#1a2a6c]' : 'w-2.5 bg-gray-300'}`} />
          ))}
        </div>
        <button onClick={next} disabled={current >= maxIndex} className="w-11 h-11 rounded-full border border-gray-200 flex items-center justify-center hover:border-[#1a2a6c] hover:text-[#1a2a6c] disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
  ...
  return (
    <div className="min-h-screen overflow-hidden">
      ...
      <section className="relative bg-[#f0f0f0] overflow-hidden">
        <div className="absolute right-[8%] top-1/2 -translate-y-1/2 w-[44vw] h-[44vw] max-w-[500px] max-h-[500px] rounded-full bg-rose-100/60 pointer-events-none hidden lg:block" style={{ zIndex: 0 }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="inline-block mb-6">
                <span className="bg-rose-100 text-rose-600 px-5 py-2 rounded-full text-sm font-semibold tracking-wide">Factory Direct: Save Up to 45%</span>
              </motion.div>

              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-[#1a1a2e] leading-[1.05] mb-6">
                Sleep Better.<br />Live Better.
              </motion.h1>

              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="text-gray-600 text-base sm:text-lg md:text-xl mb-10 max-w-lg">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3 }} className="flex flex-wrap gap-4 mb-12">
                <Link to="/products">
                  <button className="bg-[#1a2a6c] hover:bg-[#0f1d56] text-white font-semibold px-9 py-4 rounded-lg text-base transition-colors duration-200">Shop Now</button>
                </Link>
                <Link to="/products">
                  <button className="border-2 border-gray-300 text-[#1a1a2e] hover:bg-gray-50 font-semibold px-9 py-4 rounded-lg text-base transition-colors duration-200">Compare Mattresses</button>
                </Link>
              </motion.div>

              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.4 }} className="flex flex-wrap gap-8">
                {[{ icon: '🛡️', label: '10-Year Warranty' }, { icon: '🚚', label: 'Free Shipping' }, { icon: '✅', label: 'ISO Certified' }].map((b) => (
        ...
        <div className="w-full max-w-screen-2xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4">
            {heroFeatures.map((f, i) => (
              <motion.div key={f.title} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex flex-col items-center text-center py-10 px-6 gap-3">
        ...
        <div className="w-full px-6 lg:px-16 max-w-screen-2xl mx-auto">
          <AnimatedSection>
            <div className="flex items-center justify-between mb-10">
              ...
              <Link to="/products" className="text-gray-500 text-base font-medium flex items-center gap-1 hover:text-[#1a2a6c] transition-colors">
        ...
        <div className="w-full px-6 lg:px-16 max-w-screen-2xl mx-auto">
          <AnimatedSection>
            <div className="relative rounded-3xl overflow-hidden min-h-[260px] flex items-center">
              <img src={catLatex} alt="Festive Sale" className="absolute inset-0 w-full h-full object-cover object-center" />
              ...
              <div className="relative z-10 px-12 py-12 w-full md:w-1/2">
                ...
                <Link to="/products">
                  <button className="bg-white hover:bg-gray-100 text-[#1a2a6c] font-semibold px-9 py-3.5 rounded-full text-base transition-colors shadow-md">Shop Now</button>
                </Link>
              </div>
            </div>
          </AnimatedSection>
            ...
            {sixFeatures.map((item) => (
              <StaggerItem key={item.title}>
                <div className="flex items-start gap-5 group">
                  <div className="w-14 h-14 flex-shrink-0 rounded-xl bg-gray-100 flex items-center justify-center text-2xl group-hover:bg-blue-50 transition-colors">{item.icon}</div>
                  <div>
      ...
      <section className="py-20 bg-[#edf1f7]">
        <div className="w-full px-6 lg:px-16 max-w-screen-2xl mx-auto">
          <AnimatedSection className="text-center mb-12">
            <h2 className="text-4xl md:text-5xl font-extrabold text-[#1a1a2e] mb-4">Loved by 100,000+ Sleepers</h2>
            <div className="flex items-center justify-center gap-1 mb-2">
              {[...Array(5)].map((_, i) => (<Star key={i} className="w-6 h-6 text-gray-300" />))}
            </div>
            <p className="text-gray-500 text-base">Rated 4.9/5 Based on Verified Purchase Reviews</p>
          </AnimatedSection>
          <AnimatedSection><TestimonialSlider /></AnimatedSection>
        </div>
      </section>
        ...
        <div className="w-full px-6 lg:px-16 max-w-screen-2xl mx-auto text-center">
          <AnimatedSection>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link to="/about">
                <button className="border-2 border-gray-200 text-[#1a1a2e] hover:bg-gray-50 font-semibold px-10 py-4 rounded-lg text-base transition-colors">Know More</button>
              </Link>
            </div>
          </AnimatedSection>
        </div>
