...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`${API_URL}/admin/recent-orders`, { headers }).then(r => r.json()).catch(() => ({ data: [] })),
    ])
      .then(([dashJson, ordersJson]) => {
        if (!dashJson.success) { setError(dashJson.message || 'Failed to load stats'); return; }
        const ov = dashJson.data?.overview ?? dashJson.data ?? {};
        const recentOrders = Array.isArray(ordersJson.data) ? ordersJson.data : ordersJson.data?.orders ?? [];
        setStats({
